...
void luckyset::generateNum(const Lnode& g, Lotto& p) {
   ...
   fpl::setnow_figure(now_figure, g.num);// 한 게임 세트 정보를 바탕으로 now_figure 세팅
   ...
   db_figure=myfunction::istherenow_figure(now_figure, p.getPattern());//db에 없으면 zero 반환
   ...
   if(!onNoff) {
         ...
         else cout << " >> aliveNum's setting is not working rightly\n";
         //aliveNum_checking
         int _ssz1=_aliveNum;
         cout << "- aliveNum_checking \n";
         for(int p=0; p<_ssz1; ++p)
            cout << aliveNum[p] << ' ';
         ...
         while(addNum&&count1++<n) {
               ...
               while(aliveOrnot&&count2++<_aliveOrnot) {
                  int ch5=0;
                  ...
                  of1 << " |" << endl;
                  //중복되는지 확인
                  ch4=myfunction::isthereNnum(tLnode, this.getNewNumber());
                  //중복된 게 아니라면
